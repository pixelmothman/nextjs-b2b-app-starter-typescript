...
export default function StyledDropzone(props: any) {
  ...
  const uploadImage = async (p: string, t: string, fileToUpload: FileWithPreview) => {
    ...
    if (error) {
      toast.custom(t => (
        <div className="w-fit h-fit mb-2 mr-2 flex flex-row items-center justify-center gap-2 p-4 bg-neutral-50 border border-neutral-800 rounded-sm shadow-sm shadow-neutral-800">
          ...
          <p className="text-sm font-bold text-neutral-800">Image upload error</p>
        </div>
      ));
    } else if (data) {
      toast.custom(t => (
        <div className="w-fit h-fit mb-2 mr-2 flex flex-row items-center justify-center gap-2 p-4 bg-neutral-50 border border-neutral-800 rounded-sm shadow-sm shadow-neutral-800">
          ...
          <p className="text-sm font-bold text-neutral-800">Image uploaded successfully!</p>
        </div>
      ));
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col p-5 rounded-sm bg-white border border-neutral-800 shadow-sm overflow-y-auto">
      <div className="w-full h-fit flex flex-col gap-2">
        <h2 className="text-2xl font-bold text-neutral-800">Upload an image</h2>
        <p className="text-neutral-800 mb-4">Upload up to 1 image.</p>
      </div>
      ...
      <form action={formAction} className="h-full flex flex-col gap-4 justify-between">
        <div className="flex flex-col gap-2">
          ...
          <input
            autoComplete="off"
            type="hidden"
            value={files[0]?.path?.split('.').pop()}
            id="image-type"
            name="image-type"
            className="form-input w-full h-10 px-4 py-2 rounded-md bg-neutral-100 text-neutral-800 outline-0 ring-0 border-0 focus-visible:ring-black"
          />
        </div>
